...
def generate_video_trims(
    ...
    for i, (start, end) in enumerate(timestamps):
        (
            ffmpeg.input(
                video_path, ss=max(start - offset, 0.0), to=min(end + offset, duration)
            )
            .output(
                f"{save_folder}/{temp_files[i]}",
                vcodec="libx264",
                acodec="aac",
                strict="experimental",
            )
...
def merge_trim_files(trim_file_list_path: str, save_folder: str, save_name: str) -> str:
    ...
    (
        ffmpeg.input(trim_file_list_path, format="concat", safe=0)
        .output(
            merged_video_path, vcodec="libx264", acodec="aac", strict="experimental"
        )
        .global_args("-loglevel", "error")
        .run(overwrite_output=True)
    )
...
def generate_merged_video_for_bullet_dicts(
    video_path: str,
    bullet_dicts: list[dict],
    merged_file_idx: int,
    trims_save_folder: str,
    merged_files_save_folder: str,
) -> str:
    ...
    for b_idx, bullet_dict in enumerate(bullet_dicts):
        ...
        trim_file_list_path = generate_video_trims(
            video_path, timestamps, trims_save_folder, b_idx
        )
    merged_video_path = merge_trim_files(
        trim_file_list_path,
        merged_files_save_folder,
        f"{filename}_{merged_file_idx}.mp4",
    )
